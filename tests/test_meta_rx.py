...
class MetaGetTest_Rx(TestCase):
    ...
    def test_get_valid_leaf(self, s):
        ...
        result = icloudpd.meta_rx._get(source, "level1", "level2").pipe(ops.to_iterable()).run()
    ...
    def test_get_long_path_returns_empty(self, s):
        source = defaultdict(dict)
        source["level1"]["level2"] = s
        result = icloudpd.meta_rx._get(source, "level1", "level2", "level3").pipe(ops.to_iterable()).run()
    ...
    def test_get_short_path_returns_dict_node(self, s):
        ...
        result = icloudpd.meta_rx._get(source, "level1", "sublevel11", "sublevel12").pipe(ops.to_iterable()).run()
        self.assertEqual(result,[{ "level2": s}])

class MetaTest_Rx(TestCase):
    ...
    def test_get_filename_missing(self, s):
        ...
        pair[0]["recordName"] = s
        result = icloudpd.meta_rx._get_filename(pair).pipe(ops.to_iterable()).run()
        self.assertEqual(len(result),1)

    def test_get_asset_date_missing(self):
        pair = (defaultdict(dict), defaultdict(dict))
        result = icloudpd.meta_rx._get_asset_date(pair).pipe(ops.to_iterable()).run()
    ...
    def test_get_id(self, s):
        pair = (defaultdict(dict), defaultdict(dict))
        pair[0]["recordName"] = s
        result = icloudpd.meta_rx._get_id(pair).pipe(ops.to_iterable()).run()
        self.assertEqual(result,[s])
    ...
    def test_get_url(self, t, s, l):
        ...
        result = icloudpd.meta_rx._get_url(pair).pipe(ops.to_iterable()).run()
