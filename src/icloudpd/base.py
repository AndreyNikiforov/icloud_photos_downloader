...
def build_lp_filename_generator(
    ...
    return (
        ...
        if lp_filename_policy == "original"
...
def raw_policy_generator(
    ...
    if raw_policy == "as-is":
        ...
        raise ValueError(
            f"policy was provided with unsupported value of '{raw_policy}'"
        )
...
def password_provider_generator(
    ...
    def _map(
        provider: str,
    ) -> Tuple[Callable[[str], Optional[str]], Callable[[str, str], None]]:
        if provider == "console":
            ...
            raise ValueError(
                f"password provider was given an unsupported value of '{provider}'"
            )
...
@click.option(
    ...
    type=click.Choice(
        ["name-size-dedup-with-suffix", "name-id7"], case_sensitive=False
    ),
...
def main(
    ...
    with logging_redirect_tqdm():
        ...
        if watch_with_interval and (
            list_albums or only_print_filenames
        ):  # pragma: no cover
            print(
    ...
    def state_(icloud: PyiCloudService) -> Callable[[Counter, PhotoAsset], bool]:
        def download_photo_(counter: Counter, photo: PhotoAsset) -> bool:
            ...
            try:
                ...
                print(
                    "icloudpd has saved the photo record to: "
            ...
            for download_size in size:
                if (
                    download_size not in versions
                    and download_size != AssetVersionSize.ORIGINAL
                ):
                    if force_size:
                        logger.error(
                ...
                if not file_exists and download_size == AssetVersionSize.ORIGINAL:
                    ...
                    # Now we match the behavior of iCloud for Windows:
                    # IMG_1234.jpg
                ...
                if file_exists:
                    if file_match_policy == FileMatchPolicy.NAME_SIZE_DEDUP_WITH_SUFFIX:
                        # for later: this crashes if download-size medium is
                        # specified
                        file_size = os.stat(
                    ...
                    if only_print_filenames:
                        ...
                        if download_result:
                            if (
                                ...
                                and not exif_datetime.get_photo_exif(
...
def internal_error_handle_builder(
    logger: logging.Logger,
) -> Callable[[Exception, int], None]:
...
def core(
    ...
    if list_libraries:
        ...
        while True:
            ...
            logger.debug(
                "Looking up all photos%s from album %s...", videos_phrase, album
            )
