...
def parse_trusted_phone_numbers_response(
    response: _Response,
...
def parse_trusted_phone_numbers_payload(content: str) -> Sequence[TrustedDevice]:
    ...
    return list(
        (item for item in map(_map_to_trusted_device, numbers) if item is not None)
    )
...
def _oauth_redirect_header(domain: str) -> Mapping[str, str]:
    return {
        "X-Apple-OAuth-Redirect-URI": (
            "https://www.icloud.com.cn" if domain == "cn" else "https://www.icloud.com"
        ),
    }
...
class _TrustedPhoneContextProvider(
    _DomainProvider, _OAuthSessionProvider, Protocol
): ...
...
def build_trusted_phone_numbers_request(
...
def build_send_sms_code_request(
    context: _TrustedPhoneContextProvider, device_id: int
) -> Request:
    """Builds a request for the list of trusted phone numbers for sms 2fa"""
...
def build_verify_sms_code_request(
    context: _TrustedPhoneContextProvider, device_id: int, code: int
) -> Request:
    """Builds a request for the list of trusted phone numbers for sms 2fa"""
