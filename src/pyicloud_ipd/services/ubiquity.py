...
class UbiquityService(object):
    ...
    def get_node_url(self, id, variant="item"):
        return self._service_root + self._node_url % (self.params["dsid"], variant, id)
    ...
    def get_children(self, id):
        request = self.session.get(self.get_node_url(id, "parent"))
        items = request.json()["item_list"]
        return [UbiquityNode(self, item) for item in items]

    def get_file(self, id, **kwargs):
        request = self.session.get(self.get_node_url(id, "file"), **kwargs)
        return request
